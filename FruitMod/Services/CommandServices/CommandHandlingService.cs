...
namespace FruitMod.Services
{
    ...
    public class CommandHandlingService
    {
        ...
        public Task HandleCommandAsync(SocketMessage messageParam)
        {
            ...
            _ = Task.Run(async () =>
              {
                  if (!(messageParam is SocketUserMessage message) || messageParam.Author.IsBot && messageParam.Author.Id != _client.CurrentUser.Id) return;
                  ...
                  if (guild != null)
                  {
                      ...
                      var argPos = 0;
                      if (!(prefixes.Any(x => message.HasStringPrefix(x, ref argPos)) || message.HasMentionPrefix(_client.CurrentUser, ref argPos))) return;
                      ...
                      switch (result.Error)
                      {
                          ...
                          case CommandError.BadArgCount:
                              await context.Channel.SendMessageAsync($"You did not supply the correct amount of arguments for this command. See prefix info <command>");
                              break;
