    ...
    public class AudioService
    {
        ...
        public async Task JoinAsync(SocketGuildUser user, ulong guildId, string title, SocketTextChannel channel)
        {
            ...
            if (_player == null)
                ...
                if (_player.Playing == false) await _manager.JoinAsync((user as IVoiceState).VoiceChannel);
            ...
            if (track == null)
                ...
                await channel.SendMessageAsync($"Track {track.Title} by {track.Author} has been found. Length: {track.Length}");
        ...
        public async Task PlayAsync(ulong guildId, SocketTextChannel channel, LavalinkPlayer player)
        {
            ...
            if (!_player.Playing)
            {
                if (dbo.MusicQueue.TryDequeue(out string track))
                {
                ...
                { await channel.SendMessageAsync("Track failed! Is the queue empty?"); return; }
            }
        }
        ...
        {
            var dbo = _db.GetById<GuildObjects>(guildId);
            if (dbo.MusicQueue.TryPeek(out string track))
